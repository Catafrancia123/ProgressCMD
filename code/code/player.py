...
def startGame(systemName, startLevel, proLevel, systemlogo):
    ...
    while True:
        ...
        #april fools update!!!!
        aprilfools = random.randint(0, 100)
        if startLevel >= 1 and startLevel <= 25:
            continue
        elif startLevel >= 25 and startLevel <= 50:
            aprilfools = random.randint(0, 50)
            continue
        elif startLevel >= 50:
            aprilfools = random.randint(0, 25)
            continue

        if aprilfools == 25:
            clear()
            raise SystemError("HAPPY APRIL FOOLS LMAOOOOOOOOO!!!")
